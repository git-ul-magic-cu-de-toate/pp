...
match_rule(Tokens, _, rule(R, _, _, _, _)) :- R = Tokens.
...
find_matching_rules(Tokens, Rules, _, MatchingRules) :- findall(X, (match_rule(Tokens, _, X), member(X, Rules)), MatchingRules).
...
select_answer(Tokens, _, _, Answer, Actions) :- rules([Cuvant | _], ListaR), 
												member(Cuvant, Tokens), 
												find_matching_rules(Tokens, ListaR, _, [rule(_, [Answer | _], Actions, _, _) | _]).
