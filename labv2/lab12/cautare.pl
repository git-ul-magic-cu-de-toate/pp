...
safeMisionari(M, C) :- M >= C, M >= 0, C >= 0.
safeMisionari(0, C) :- C >= 0.
...
parseState(state(M, E, CE, CV), M, ME, CE, MV, CV) :- ME is 3 - MV.
...
next_state(misionari, state(M, ME, CE, MV, CV), state(M1, ME1, CE1, MV1, CV1)) :-
    (M = est, M1 = vest, boat(MB, CB), ME1 is ME - MB, CE1 is CE - CB, MV1 is MV + MB, CV1 is CV + CB);
    (M = vest, M1 = est, boat(MB, CB), ME1 is ME + MB, CE1 is CE + CB, MV1 is MV - MB, CV1 is CV - CB),
    safe(ME1, CE1), safe(MV1, CV1).
