...
;; Restricții:
;; - se va implementa obligatoriu recursiv, după modelul implementării lui rev DA
;; - folosiți cond
;; - folosiți list? pentru a verifica dacă elementul procesat este o listă DA

(define (deep-rev L)
  (
   if (null? L) '() 
         (cons (cons (cdr (car L)) (car (car L))) (deep-rev(cdr L)))       
  ))
(deep-rev '((1 . 2) (3 . 4) (5 . 6)))
(deep-rev (reverse '((1 . 2) (3 . 4) (5 . 6))))
...
(define (numberr L)
  (
   if (null? L)
      0
      (
       ;; stim deja ca e lista de liste
       ;; nu mai trebuie sa testezi asta
       ;; deci
       if (equal? (car L) (rev(car L))) (+ (numberr(cdr L)) 1) (+ (numberr(cdr L)) 0)
          )
  )
  )
...
;; a) După ce variabilă/variabile e util să faceți recursivitatea? lista de baze
;;    - ar ajuta să știți dacă n-1 este palindrom în toate bazele din Bases? nu.
;;    - ar ajuta să știți dacă n este palindrom în toate bazele din (cdr Bases)? da
...
;;       pentru mine cu recursivitate dupa lista de baze, conditia de oprire e ca lista sa aiba elemente
;;       intoarce lista vida ca pun in lista liste cu cifrele din fiecare baza =)) si apoi folosesc palindrome-list? 
...
;;      rezultatul final -> e, de fapt, ce intoarce palindrome-list =))
;; c) Care este apelul recursiv (e obligatoriu să aveți minim unul), și care este formula prin care 
;;    din apelul recursiv se obține rezultatul final? -> cred ca am zis mai sus cum fac aici so yeah, am raspuns si la asta

(define (all-palindromes? n Bases)
  ;; return true or false
  ;; mai intai il transformi in baza
  ;; apoi testezi sa vezi ca e palin in baza aia si numeri
  ;; Bases e lista
  (equal? (numm n Bases) (length Bases))
)

(define (numm n Bases)
  (if (null? Bases)
      0
  (if (equal? (num->base n (car Bases)) (rev (num->base n (car Bases))))
      (+ (numm n (cdr Bases)) 1) (+ (numm n (cdr Bases)) 0)
  )
)
  )
...
(exercițiul 6.0 : 0 puncte) ;; vai ;_; nu. 
...
(exercițiul 7 : 2 puncte) ;; baby ca se aseamana cu 6
;; folosesc 6 pe masura ce scad n si le bag intr-o lista cand 6 intoarce length de whatever si ma opresc cand n e 0
  ...
  (if (equal? n 0)
      ...
      (
       if (all-palindromes? n Bases)
          ...
          (append (palindromes-to-n (sub1 n) Bases) '())
          )
      )
  )
...
;; deci, aici pun toate bazele intr-o lista pe care o parcurg si vad daca un numar
;; pe care il tot cresc e palindrom intr-o baza din acelea si le numar pe bazele in care e palindrom

;; se aseamana cu 5,6,7 numai ca trebuie sa am putina grija
;; daca numarul e mai mare decat b, atunci afisez. aceasta e conditia de oprire

;; mai intai numarul care e palindrom in minim b baze, apoi il cresc
;; deci mai intai fac functia de verificare
;; daca se bazeaza pe celelalte, se face deci cu copy-pasye

(define my_list '(2 3 4 5 6 7 8 9 10))
(define (first-b-pal start b)
  ;; de gasit conditie de oprire =/
  ;; asa
  ;; deci
  (if (
      >= (numm start my_list) b)
      start
...
;; ma opresc cand gasesc o lista cu mai putine elemente decat lista optima
;; lista optima este atunci camd e palindromica de lungime maxima
