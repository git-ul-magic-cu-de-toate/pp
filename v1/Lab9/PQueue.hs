...
data D = DC1 Int | DC2 Int Int deriving Eq

data Subject = PA | PP | PC | ED | CN deriving (Show, Eq, Ord)

codeCracker::Subject -> Subject -> Subject -> String
codeCracker s1 s2 s3 = if (s1 < s2) && (s3 == s2) 
                        then "Success"
                        else "Failure"

data TList = Nil | ConsI Int TList | ConsS String TList

-- newtype Mytype = M (Int -> Int)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert elem (LPQ qlist)
      | null qlist = LPQ [elem]
      | fst elem >= fst (head qlist) = LPQ (elem : qlist)
      | fst elem < fst (head qlist) = LPQ ((head qlist) : (toList (insert elem (LPQ (tail qlist)))))

    top (LPQ qlist)
      | null qlist = Nothing
      | otherwise = Just (head qlist)

    pop (LPQ qlist) = LPQ (tail qlist) 

    isEmpty (LPQ qlist) = null qlist
...
merge tree1@(Node rank1 (prio1, val1) l1 r1) tree2@(Node rank2 (prio2, val2) l2 r2)
  | prio1 <= prio2 = merge tree2 tree1
  | rank l1 >= rank tree3 = (Node ((rank tree3) + 1) (prio1, val1) l1 tree3)
  | otherwise = (Node ((rank l1) + 1) (prio1, val1) tree3 l1)
    where 
      tree3 = merge r1 tree2

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty _) = []
inorder (Node rank elem l r) = inorder l ++ [elem] ++ inorder r 
...
instance (Ord a) => PQueue LeftistPQ a where
    ...
    insert elem tree = merge (Node 1 elem empty empty) tree

    top qlist
      | isEmpty qlist = Nothing
      | otherwise = Just (nodeVal qlist)

    pop qlist = merge (left qlist) (right qlist)
