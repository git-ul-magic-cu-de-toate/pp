...
myConcat([Head | Tail], List2, [Head | List]) :- myConcat(Tail, List2, List).
...
myReverse([], []).
myReverse([Head | Tail], RevList) :- myReverse(Tail, RevTail), myConcat(RevTail, [Head], RevList).
...
extract([Head | Tail], 0, End, [Head | Range]) :- extract(Tail, 0, End - 1, Range).
extract([_ | Tail], Start, End, Range) :- PrevStart is Start - 1, extract(Tail, PrevStart, End - 1, Range).
...
factorial(N, 1) :- N =< 1, !.
factorial(N, Fact) :- PrevN is N - 1, factorial(PrevN, PrevFact), Fact is PrevFact * N.
...
factorial2(1, 1).
factorial2(N, Fact) :- factorial2(PrevN, PrevFact), N is PrevN + 1, Fact is PrevFact * N.
...
descendantOf(NodX, NodY) :- arc(NodY, NodX).
descendantOf(NodX, NodY) :- arc(AltNod, NodX), descendantOf(AltNod, NodY).
...
descendants(Nod, N) :- arc(Nod, AltNod), \+ (arc(Nod, AltNod2), AltNod \= AltNod2),
                       descendants(AltNod, AltNodN), N is AltNodN + 1.
descendants(Nod, N) :- arc(Nod, AltNod), arc(Nod, AltNod2), AltNod \= AltNod2,
                       descendants(AltNod, AltNodN), descendants(AltNod2, AltNod2N),
                       N is AltNodN + AltNod2N + 2.
