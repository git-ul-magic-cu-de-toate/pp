...
(struct bird-info (x y yspeed))

(define (get-initial-bird)
  (bird-info bird-x bird-initial-y 0))
...
(struct pipe-info (x y))

(define (get-initial-pipe)
  (pipe-info scene-width (+ added-number (random random-threshold))))
...
(struct state-info (bird-info pipes gravity momentum scrollspeed score))

(define (get-initial-state)
  (state-info (get-initial-bird)
              (list (get-initial-pipe))
              initial-gravity
              initial-momentum
...
(define (get-gravity state)
  (state-info-gravity state))

(define (get-momentum state)
  (state-info-momentum state))

(define (get-scrollspeed state)
  (state-info-scrollspeed state))
...
(define (next-state-bird bird gravity)
  (struct-copy bird-info bird [y (+ (get-bird-y bird) (get-bird-v-y bird))]
               [yspeed (+ (get-bird-v-y bird) gravity)]))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy bird-info bird [yspeed (- momentum)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy state-info current-state
                   [bird-info (next-state-bird-onspace (get-bird current-state)
                                                       (get-momentum current-state))])
      current-state))
...
(define (move-pipes pipes scroll-speed)
  (map (λ (y)
         (struct-copy pipe-info y [x (- (get-pipe-x y) scroll-speed)]))
         pipes))
...
(define (clean-pipes pipes)
  (filter (λ (x) (>= (+ (get-pipe-x x) pipe-width) 0)) pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (append pipes (list (pipe-info (+ (+ pipe-width pipe-gap) (get-pipe-x (car (reverse pipes))))
...
(define (invalid-state? state)
  (let ((bird (get-bird state)))
    (or (check-ground-collision bird) (check-pipe-collisions bird (get-pipes state)))))
...
(define (check-pipe-collisions bird pipes)
  (let loop ((p pipes) (bird-x (get-bird-x bird)) (bird-y (get-bird-y bird)))
    (cond ((null? p) #f)
          ((check-collision-rectangles
             (make-posn bird-x bird-y)
             (make-posn (+ bird-x bird-width) (+ bird-y bird-height))
             (make-posn (get-pipe-x (car p)) 0)
             (make-posn (+ (get-pipe-x (car p)) pipe-width) (get-pipe-y (car p)))) #t)
          ((check-collision-rectangles
             (make-posn bird-x bird-y)
             (make-posn (+ bird-x bird-width) (+ bird-y bird-height))
             (make-posn (get-pipe-x (car p)) (+ (get-pipe-y (car p)) pipe-self-gap))
             (make-posn (+ (get-pipe-x (car p)) pipe-width) pipe-height)) #t)
          (else (loop (cdr p) bird-x bird-y)))))
...
(define (next-state state)
  (state-info (next-state-bird (get-bird state) (get-gravity state))
              (next-state-pipes (get-pipes state) (get-scrollspeed state))
              (get-gravity state)
              (get-momentum state)
              (get-scrollspeed state)
              (+ (get-score state) 0.1)))
...
(define (draw-frame state)
  (let* ((bird (get-bird state)) (score-image (score-to-image (get-score state))))
    (place-images
     (list bird-image ground-image score-image)
     (list (make-posn (+ (get-bird-x bird) (quotient bird-width 2))
...
(define (place-pipes pipes scene)
  (if (null? pipes)
      scene
      (let* ((pipe (car pipes))
         (pipe-x (get-pipe-x pipe))
         (pipe-y (get-pipe-y pipe))
         (down-pipe-height (- (- scene-height pipe-self-gap) pipe-y))
         (down-pipe-image (rectangle pipe-width down-pipe-height "solid" "green"))
         (up-pipe-image (rectangle pipe-width pipe-y "solid" "green")))
    (if (null? pipes)
        scene
        (place-pipes
         (cdr pipes)
         (place-image down-pipe-image
                      (+ pipe-x (quotient pipe-width 2))
                      (- scene-height (quotient down-pipe-height 2))
                      (place-image up-pipe-image
                                   (+ pipe-x (quotient pipe-width 2))
                                   (quotient pipe-y 2)
                                   scene)))))))
; Bonus
