...
safe(_, []) :- !.
safe(X/Y/S, [X1/Y1/S1 | Others]) :- (S \== S1; (X \== X1, Y \== Y1, S == S1)), safe(X/Y/S, Others).
...
% problemă.
safeMisionari(NM, NC) :- once(NM == 0; NM >= NC).
...
% dată.
parseState(state(Mal, NMEst, NCEst, NMVest, NCVest), Mal, NMEst, NCEst, NMVest, NCVest).
...
next_state(misionari, S1, S2) :- parseState(S1, Mal, NMEst, NCEst, NMVest, NCVest),
                                 Mal == vest, boat(NM, NC), NM =< NMVest, NC =< NCVest,
                                 NMVest2 is NMVest - NM, NCVest2 is NCVest - NC,
                                 NMEst2 is NMEst + NM, NCEst2 is NCEst + NC,
                                 safeMisionari(NMVest2, NCVest2), safeMisionari(NMEst2, NCEst2),
                                 S2 = state(est, NMEst2, NCEst2, NMVest2, NCVest2).

next_state(misionari, S1, S2) :- parseState(S1, Mal, NMEst, NCEst, NMVest, NCVest),
                                 Mal == est, boat(NM, NC), NM =< NMEst, NC =< NCEst,
                                 NMEst2 is NMEst - NM, NCEst2 is NCEst - NC,
                                 NMVest2 is NMVest + NM, NCVest2 is NCVest + NC,
                                 safeMisionari(NMEst2, NCEst2), safeMisionari(NMVest2, NCVest2),
                                 S2 = state(vest, NMEst2, NCEst2, NMVest2, NCVest2).
...
dfs([], Parent, Visited) :- reverse(Parent, Visited).
dfs([Head | Tail], Parent, Visited) :- \+ arc(Head, _), dfs(Tail, Parent, [Head | Visited]).
dfs([Head | Tail], Parent, Visited) :- arc(Head, Child), dfs(Child, ParentofChild, [Head | Visited]),
									   reverse(VisitedChild, ParentofChild),
									   dfs(Tail, Parent, VisitedChild).

preorder(N, [N]) :- \+ arc(N, _).
preorder(N, [N | ParentofChild]) :- arc(N, Child), dfs(Child, ParentofChild, []).
