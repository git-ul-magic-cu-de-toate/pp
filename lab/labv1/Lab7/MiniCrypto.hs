...
arithmetic a b = iterate (\a -> a + b) a
geometric :: Num a => a -> a -> [a]
geometric a b = iterate (\a -> a * b) a
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms x = map (fromIntegral . fst) lst
	where 
		g = mkStdGen x
		start = next g
		lst = iterate getNext start
		getNext (value, g) = next g
{-
    4. (1p)
...
wordToAlpha x = chr (((fromIntegral x) `mod` 26) + 97)
{-
    5. (1p)
...
randomAlphaKey x = map wordToAlpha (randoms x)
{-
  6. (3p)
...
tableToFunc table = check
	where
		check c = snd $ head $ filt c
		filt b = filter (\x -> (fst x) == b) table
