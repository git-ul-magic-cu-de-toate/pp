...
data IntNumber = IntNumber { value1 :: Int } deriving Show

instance Number IntNumber where
  increment nr = nr {value1 = value1 nr + 1}
  decrement nr = nr {value1 = value1 nr - 1}

normalizeV :: Vector -> Vector
normalizeV var@(V a1 a2 a3) = (V (a1 / (lengthV var)) (a2 / (lengthV var)) (a3 / (lengthV var)))
...
orthogonalV list =  (length l) == 0
  where
    scal = (map (dotV (head list)) (tail list))
    l = [ y | y <- scal, (not (y == 0))]
...
{-
 2. (4p) Liste imbricate
  ...
  (1p) flatten
-}
data SList a = ConsSList [SList a] | Basic a deriving Show

emptySList :: SList a
emptySList = ConsSList []

consElem :: a -> SList a -> SList a
consElem elem (Basic l1) = ConsSList ([Basic elem, Basic l1])
consElem elem (ConsSList lst) = ConsSList ((Basic elem) : lst)

consList :: SList a -> SList a -> SList a
consList l1 (Basic l2) = ConsSList ([l1, Basic l2])
consList l1 (ConsSList l2) = ConsSList (l1 : l2)

headSList :: SList a -> SList a
headSList (ConsSList []) = emptySList
headSList (Basic l1) = (Basic l1)
headSList (ConsSList lst) = head lst

tailSList :: SList a -> SList a
tailSList (ConsSList []) = emptySList
tailSList (Basic l1) = emptySList
tailSList (ConsSList x) = ConsSList $ tail x

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual (ConsSList []) (ConsSList []) = True
deepEqual (Basic l1) (Basic l2) = (l1 == l2)
deepEqual (Basic l1) (ConsSList l2) = False
deepEqual (ConsSList l1) (Basic l2) = False
deepEqual (ConsSList l1) (ConsSList l2) = length [ y | y <- zipWith deepEqual l1 l2, y] /= 0

flatten :: SList a -> [a]
flatten (ConsSList []) = []
flatten (Basic l1) = [l1]
flatten lst@(ConsSList l1) = (flatten (headSList lst)) ++ (flatten (tailSList lst))
...
-}
data BST a = UndefinedNode | BSTNil | BSTNode a (BST a) (BST a) deriving Show
...
insertElem (BSTNode value left right) elem
  | elem <= value = BSTNode value (insertElem left elem) right
  |otherwise = BSTNode value left (insertElem right elem)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil elem = Nothing
findElem (BSTNode value left right) elem
  | elem == value = Just elem
  | elem <= value = findElem left elem
  | otherwise = findElem right elem

subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree (BSTNode value left right) elem1 elem2
  | findElem (BSTNode value left right) elem1 == Nothing = Nothing
  | findElem (BSTNode value left right) elem2 == Nothing = Nothing
  | elem1 <= value && elem2 <= value = subTree left elem1 elem2
  | elem1 > value && elem2 > value = subTree right elem1 elem2
  | otherwise = Just (BSTNode value left right)
