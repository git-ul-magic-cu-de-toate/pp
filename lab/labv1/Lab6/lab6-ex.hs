...
-}
primes :: Int -> [Int]
primes n = [x | x <-[2..n], length [ y | y <- [2..(x `div` 2)], x `mod` y == 0 ] == 0]
...
count [] _ = 0
count list acc
	| acc == head list = 1 + (count (tail list) acc)
	| otherwise = 0

group2 :: Eq a => [a] -> [[a]]
group2 [] = []
group2 a
	| length a > 0 = [(take nr a)] ++ (group2 (drop nr a)) 
	| otherwise = []
	where
		nr = count a (head a)
...
-}
nrOcc :: Ord a => [a] -> [(a, Int)]
nrOcc list = [ (head(x), length(x)) | x <- (group2 (sort list)) ]
...
-}
dup :: String -> String
dup string = unwords([ x ++ " " ++ x | x <- (words string)])
