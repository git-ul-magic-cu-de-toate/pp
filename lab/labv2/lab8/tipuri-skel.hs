...
{- interesant. V e constructorul si nu ai v1 si v2, ai v si v. -}
check1 :: TestData
check1 = let
      v1 = V 1 (-1) 0
      v2 = V 1 1 0
...
data BST a = BNode a (BST a) (BST a) | BSTNil deriving Show

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil x = BNode x BSTNil BSTNil
insertElem (BNode val st dr) x
    | x == val = (BNode val st dr)
    | x < val = BNode val (insertElem st x) dr
    | otherwise = BNode val st (insertElem dr x)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem (BNode val st dr) x
    | x == val = Just val
    | x < val = findElem st x
    | otherwise = findElem dr x

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BNode val st dr) = inorder st ++ [val] ++ inorder dr

size :: BST a -> Int
size BSTNil = 0
size (BNode val st dr) = 1 + size st + size dr

height :: BST a -> Int
height BSTNil = 0
height (BNode val st dr) = 1 + max (size st)  (size dr)

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil 
isBalanced = if height st == height dr then True else False
