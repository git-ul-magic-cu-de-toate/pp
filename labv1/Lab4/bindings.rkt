...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((a (get-line-segment)) (b (get-start-point a)) (c (get-end-point a)))
    (let ((d (- (car c) (car b))) (e (- (cdr c) (cdr b))))
      (sqrt (+ (* d d) (* e e))))))
...
(define (distance p1 p2)
  (compute-length (λ () (cons p1 p2)) get-start-point get-end-point))
...
(define (compute-perimeter points)
  (let loop ((p (append points (list (car points)))) (perim 0))
    (if (empty? (cdr p))
        perim
        (loop (cdr p) (+ perim (distance (car p) (cadr p)))))))
...
(define (3-sequence-max numbers separator)
    (let*-values (((a b) (splitf-at numbers (λ (x) (not (equal? x separator))))) ((c d) (splitf-at (cdr b) (λ (y) (not (equal? y separator))))))
      (if (< (sum a) (sum c))
          (if (< (sum c) (sum d))
              (sum d)
              (sum c))
